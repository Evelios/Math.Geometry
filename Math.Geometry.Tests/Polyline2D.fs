module Math.Geometry.Tests.Polyline2D
...
let ``Centriod is None if polyline is empty`` =
    ...
    Polyline2D.centroid emptyPolyline |> Test.equal None
...
let ``Centroid of zero length polyline is the same point`` (point: Point2D<Meters, TestSpace>) =
    ...
    Prop.forAll tGen (fun reps ->
        let singlePointLine = List.replicate reps point |> Polyline2D.fromVertices

        Polyline2D.centroid singlePointLine |> Test.equal (Some point))
...
let ``Centroid of single line segment is middle of endpoints``
    ...
    Polyline2D.centroid singleLine |> Test.equal (Some expectedCentroid)
...
let ``Centroid of a right angle is between the two sides`` (armLength: Length) =
    ...
    let expectedCentroid = Point2D.xy (0.75 * armLength) (0.25 * armLength)

    Polyline2D.centroid angle |> Test.equal (Some expectedCentroid)
...
let ``Centroid of an open square is skewed to closed side`` (sideLength: Length) =
    ...
    let expectedCentroid = Point2D.xy (sideLength / 2.) (sideLength * 2. / 3.)

    Polyline2D.centroid squareLine |> Test.equal (Some expectedCentroid)
...
let ``Centroid of a closed square is mid-point`` (sideLength: Length) =
    ...
    let expectedCentroid = Point2D.xy (sideLength / 2.) (sideLength / 2.)
